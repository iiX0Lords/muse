...
export default class AddQueryToQueue {
  ...
  public async addToQueue({
    ...
    if (newSongs.length === 1) {
      await interaction.editReply(`sure nigga, **${firstSong.title}** added to the${addToFrontOfQueue ? ' front of the' : ''} queue${skipCurrentTrack ? 'and current track skipped' : ''}${extraMsg}`);
    } else {
      await interaction.editReply(`sure nigga, **${firstSong.title}** and ${newSongs.length - 1} other songs were added to the queue${skipCurrentTrack ? 'and current track skipped' : ''}${extraMsg}`);
    }
  }
